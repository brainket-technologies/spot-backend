...
class RestaurantController extends Controller
{
    ...
    // this is my latest code get_details create a new key (open) 2025-04-19
    public function get_details($id)
    {
        $restaurant = RestaurantLogic::get_restaurant_details($id);
        if($restaurant)
        {
            ...
            $restaurant_array = is_object($restaurant) ? $restaurant->toArray() : $restaurant;
            
            // Create a new array with keys in the desired order
            $ordered_restaurant = [];
            
            // Add all fields up to package_id
            foreach ($restaurant_array as $key => $value) {
                $ordered_restaurant[$key] = $value;
                
                // After package_id, insert the open field
                if ($key === 'package_id') {
                    $ordered_restaurant['open'] = 1; // Or calculate this value as needed
                    break;
                }
            }
            ...
            foreach ($restaurant_array as $key => $value) {
                if (!isset($ordered_restaurant[$key])) {
                    $ordered_restaurant[$key] = $value;
                }
            }
            
            return response()->json($ordered_restaurant, 200);
        }

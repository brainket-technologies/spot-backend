...
class ConfigController extends Controller
{
    ...
    // From Khushnasib code 
    public function get_zone_new(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        try {
            // Instead of using the spatial library's Point class, we'll use raw MySQL spatial functions
            // This avoids parameter count issues with ST_GeomFromText
            
            $lat = $request->lat;
            $lng = $request->lng;
            
            // Use raw DB query with correct ST_GeomFromText syntax
            $zones = DB::table('zones')
                ->whereRaw("ST_Contains(coordinates, ST_GeomFromText(CONCAT('POINT(', ?, ' ', ?, ')'), 4326))", [$lng, $lat])
                ->select([
                    'id', 
                    'status', 
            ...
            // Filter active zones
            $data = array_values(array_filter($zones->toArray(), function ($zone) {
            ...
            \Log::error('Zone detection error: ' . $e->getMessage());
            
            return response()->json([
                'errors' => [
                    ['code' => 'server_error', 'message' => translate('messages.server_error')]
                ]
            ], 500);
        }
 }
    ...
    // New Code By Faizu 
    public function appHeaderBannerUri()
    {
        return response()->json([
            "res" => "success",
            "msg" => "found",
